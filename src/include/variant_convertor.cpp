    ...
    switch(data.type()) {
        ...
        case QVariant::Map: {
                ...
                for(QVariantMap::Iterator entry = map.begin(); entry != map.end(); entry++)
                    res = res % LSTR(", ") % entry.key() % ':' % variantToStr(entry.value());
        ...
        case QVariant::Hash:
            {
                ...
                for(QVariantHash::Iterator entry = map.begin(); entry != map.end(); entry++)
                    res += LSTR(", ") % entry.key() % ':' % variantToStr(entry.value());
        ...
        case QVariant::Bool:
            { res = data.toBool() ? LSTR("true") : LSTR("false"); break;}
        case QVariant::Char:
